...
export function SensoryFeatures({ compact = false }: { compact?: boolean }) {
  return (
    <motion.div
      ...
      className={`w-full ${compact ? "px-2" : "px-3 pb-6 sm:px-6 sm:pb-8"}`}
    >
        ...
        <div className="relative">
          ...
          <div className="relative parchment-panel overflow-hidden shadow-magic">
            ...
            <div
              className={
                compact
                  ? "relative px-2 pb-2 pt-4 sm:px-6 sm:pb-4 sm:pt-6"
            ...
            >
              <h2
                className={`flex items-center justify-center gap-1 text-center font-display font-black text-forest-deep ${
                  compact
                    ? "mb-2 text-sm sm:mb-4 sm:text-base md:text-lg"
              ...
              <div
                className={`flex gap-2 overflow-x-auto pb-2 scrollbar-none snap-x snap-mandatory sm:gap-4 lg:grid lg:grid-cols-5 lg:overflow-visible lg:snap-none ${
                  compact ? "" : "sm:gap-6"
                }`}
                style={{
                ...
                {features.map((f, i) => (
                  <div
                    ...
                    className={`shrink-0 snap-center ${
                      compact
                        ? "w-[140px] min-w-[140px] sm:w-auto sm:min-w-[160px]"
                        : "w-[160px] min-w-[160px] sm:w-auto sm:min-w-[180px]"
                    }`}
                  >
                    <FeatureCard feature={f} index={i} compact={compact} />
                  </div>
                ))}
              </div>
            </div>
...
function FeatureCard({
  ...
  return (
    ...
    >
      <div
        className={`relative mb-1.5 flex items-center justify-center overflow-hidden bg-linear-to-br from-[oklch(0.97_0.02_90)] to-[oklch(0.92_0.04_95)] shadow-card-magic ring-2 ${feature.ring} ${
          compact
      ...
      >
        <img
          ...
          className={`object-contain drop-shadow-md ${
            compact
              ? "h-12 w-12 sm:h-16 sm:w-16 md:h-17 md:w-17"
      ...
      <p
        className={`leading-relaxed text-bark/85 ${
          compact
            ? "max-w-[120px] text-[9px] sm:max-w-[140px] sm:text-[10px] md:max-w-[160px] md:text-xs"
