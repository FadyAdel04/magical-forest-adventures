...
export function orderRowToRecord(row: OrderRow): OrderRecord {
  return {
    ...
    city: row.city ?? "",
    area: row.area ?? "",
    bostaDistrictId: row.bosta_district_id,
    paymentMethod: row.payment_method ?? "cash_on_delivery",
    bostaStatus: row.bosta_status,
    trackingNumber: row.tracking_number,
    trackingUrl: row.tracking_url,
    bostaOrderSent: row.bosta_order_sent ?? false,
...
export function orderRecordToRow(
  ...
  return {
    ...
    city: order.city,
    area: order.area,
    bosta_district_id: order.bostaDistrictId,
    payment_method: order.paymentMethod,
    bosta_status: order.bostaStatus,
    tracking_number: order.trackingNumber,
    tracking_url: order.trackingUrl,
    bosta_order_sent: order.bostaOrderSent,
    status: order.status,
